...
This repository currently holds the AWS provisioning for that: a Bedrock
project that inference is billed to, an IAM policy that permits inference
against that project and nothing else, and a budget alarm.
...
The three steps are ordered. Each one needs what the previous one produced.
