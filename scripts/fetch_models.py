...
def default_model_specs() -> list[ModelSpec]:
    ...
    return [
        ModelSpec(EMBEDDING_MODEL_ID, "127 MB", "config.json", load_embedder),
        ModelSpec(RERANKER_MODEL_ID, "87 MB", "config.json", load_reranker),
    ]
...
def is_cached(repo_id: str, filename: str) -> bool:
    ...
    Args:
        ...
        filename: A file present in every snapshot of that repo, used as a
            stand-in for the whole model being cached.
